...
@pytest.mark.parametrize("scenario", [
    {
        ...
        "player1_hand": [
            (14, 'Hearts'), (13, 'Hearts'),    # A♥ K♥
            (12, 'Clubs'), (11, 'Diamonds')    # Q♣ J♦
        ],
        "player2_hand": [
            (10, 'Spades'), (10, 'Clubs'),     # 10♠ 10♣
            (9, 'Hearts'), (9, 'Diamonds')      # 9♥ 9♦
        ],
        "community_cards": {
    ...
    },
    {
        ...
        "player1_hand": [
            (7, 'Hearts'), (7, 'Diamonds'),     # 7♥ 7♦
            (6, 'Hearts'), (6, 'Diamonds')      # 6♥ 6♦
        ],
        "player2_hand": [
            (8, 'Clubs'), (8, 'Spades'),       # 8♣ 8♠
            (5, 'Clubs'), (5, 'Spades')        # 5♣ 5♠
        ],
        "community_cards": {
...
def test_omaha_must_use_two_rule():
    ...
    assert evaluation.eval != 9, "Should not have straight flush - must use exactly 2 cards from hand"
...
def test_omaha_four_card_deal(base_state_omaha):
    ...
    for player in game.players:
        assert len(player.cards) == 4, f"Player {player.name} should have exactly 4 cards in Omaha"